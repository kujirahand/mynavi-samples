...
- `visual.lifeGame.enabled`: 背景ライフゲーム表示のON/OFF
- `visual.lifeGame.density`: 初期の生物密度
- `visual.lifeGame.stepSeconds`: 世代更新間隔（秒）
- `visual.lifeGame.spawnEverySteps`: 指定ステップごとのランダム出生間隔
- `visual.lifeGame.opacity`: 表示の透明度
- `visual.lifeGame.cellSize`: セルサイズ
